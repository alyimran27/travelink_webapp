...
export default function BookingList({ user, units, pengaturan, bookings, onBuatKwitansi }) {
  ...
  const daftarBulan = useMemo(() => {
    const set = new Set();
    for (const b of bookings) for (const it of b.items || []) if (it.checkIn) set.add(bulanKey(it.checkIn));
  ...
  return (
    <div className="stack">
      ...
      <div className="card">
        {tampil.length === 0 ? (
          <Kosong
            pesan={bookings.length === 0 ? 'Belum ada booking.' : 'Tidak ada booking yang cocok dengan filter.'}
