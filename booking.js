import { n, hitungMalam, bulanKey, hariIni } from './format';

/**
 * Booking dianggap MASIH AKTIF kalau ada minimal satu unit yang belum lewat
 * tanggal check-out. Tamu yang keluar hari ini tetap dihitung aktif, karena
 * admin masih perlu mengurus proses check-out-nya.
 *
 * Perbandingan sengaja memakai teks 'YYYY-MM-DD', bukan objek Date, supaya
 * tidak tergeser satu hari di zona WITA (UTC+8).
 */
export function bookingMasihAktif(b, hari) {
  const batas = hari || hariIni();
  const items = (b && b.items) || [];
  // Booking tanpa item atau tanpa tanggal keluar dianggap aktif, supaya data
  // yang belum lengkap tetap bisa dibereskan admin dan tidak hilang diam-diam.
  if (items.length === 0) return true;
  return items.some((it) => {
    const keluar = String(it && it.checkOut ? it.checkOut : '').trim();
    if (!keluar) return true;
    return keluar >= batas;
  });
}

/**
 * Saring daftar booking sesuai peran.
 * Owner melihat seluruh riwayat. Selain owner (admin dan peran lain) hanya
 * melihat booking yang masih berjalan, sehingga riwayat lama, pola harga, dan
 * data tamu yang sudah selesai tidak ikut terbuka.
 */
export function saringBookingUntukPeran(bookings, role, hari) {
  const daftar = bookings || [];
  const owner = String(role || '').trim().toLowerCase() === 'owner';
  if (owner) return daftar;
  const batas = hari || hariIni();
  return daftar.filter((b) => bookingMasihAktif(b, batas));
}
...
export function subtotalItem(item) {
  return n(item.harga) - n(item.diskon);
}
...
/**
 * Lengkapi item booking dengan data unit dari katalog.
 * Booking menyimpan properti/tipe saat dibuat, tapi booking lama bisa saja
 * belum punya (unit belum diisi propertinya, atau unit diketik manual).
 * Di sini nilai yang kosong diambilkan dari master unit berdasarkan nomornya.
 * Nilai yang sudah ada di booking TIDAK ditimpa — riwayat tetap apa adanya.
 */
export function lengkapiDariUnit(items, units) {
  const daftar = units || [];
  if (!daftar.length) return items || [];
  const peta = new Map();
  for (const u of daftar) {
    const kunci = String(u.nomor || '').trim().toLowerCase();
    if (kunci && !peta.has(kunci)) peta.set(kunci, u);
  }
  return (items || []).map((it) => {
    const u = peta.get(String(it.unitNomor || '').trim().toLowerCase());
    if (!u) return it;
    return {
      ...it,
      properti: it.properti || u.properti || '',
      unitTipe: it.unitTipe || u.tipe || ''
    };
  });
}
...
export function hitungBooking(items, dp, pelunasan) {
  ...
  const totalBruto = list.reduce((a, i) => a + n(i.harga), 0);
