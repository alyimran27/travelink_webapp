...
export default function KwitansiPage({ user, pengaturan, bookings, terpilih, setTerpilih }) {
  ...
  const booking = terpilih ? bookings.find((b) => b.id === terpilih.id) || terpilih : null;
  ...
  async function unduh() {
    ...
    setProses(true);
    try {
      await unduhPdfKwitansi(booking, pengaturan);
  ...
  async function salin() {
    const teks = teksKwitansi(booking, pengaturan);
    try {
  ...
  if (!booking) {
    return (
      <div className="card">
        ...
        <div className="mt16">
          {daftar.length === 0 ? (
            <Kosong pesan="Belum ada booking yang bisa dibuatkan kwitansi." />
          ) : (
