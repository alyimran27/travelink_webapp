...
import { hitungBooking, jumlahUnitHitung } from './booking';
...
export function teksKwitansi(booking, s) {
  const h = hitungBooking(booking.items, booking.dp, booking.pelunasan);
  const baris = [];
  baris.push('*KWITANSI*');
  ...
  h.items.forEach((it, i) => {
    ...
    baris.push(`   ${it.malam > 0 ? it.malam + ' malam' : '1 hari'} x ${rupiah(it.harga)}${it.diskon > 0 ? ` - diskon ${rupiah(it.diskon)}` : ''} = ${rupiah(it.subtotal)}`);
  });
  baris.push('');
...
export async function unduhPdfKwitansi(booking, s) {
  ...
  autoTable(doc, {
    ...
    head: [['Unit', 'Check-in', 'Check-out', 'Qty', 'Harga', 'Jumlah']],
    body: h.items.map((it) => [
      `${it.unitNomor}${it.unitTipe ? '\n' + it.unitTipe : ''}`,
