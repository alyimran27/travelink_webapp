      ...
      if (lembarBooking) {
        ...
        for (const b of bookingLama) {
          for (const it of b.items || []) {
            kunciAda.add(kunci(b.namaTamu, it.unitNomor, it.checkIn));
          }
        }
        ...
        for (let i = 0; i < rows.length; i++) {
          ...
          try {
            ...
            if (kunciAda.has(kunci(namaTamu, unitNomor, checkIn))) {
              bookingLewat++;
              continue;
            }
            ...
            const pelunasan = angka(row['Pelunasan']) || angka(row['Harga']);
            ...
            kunciAda.add(kunci(namaTamu, unitNomor, checkIn));
            bookingMasuk++;
          } catch (err) {
...
function kunci(nama, unit, tanggal) {
  return `${String(nama).trim().toLowerCase()}|${String(unit).trim().toLowerCase()}|${tanggal}`;
}
