...
export default function Dashboard({ user, bookings, expenses, setMenu }) {
  ...
  const owner = bolehLihatUang(user.role); // hanya owner yang lihat ringkasan/KPI/tren
  ...
  const ringkas = useMemo(() => {
    // Penjagaan berlapis: admin tidak pernah menerima angka uang sama sekali,
    // bukan sekadar disembunyikan di tampilan.
    if (!owner) return { nilai: 0, setoran: 0, malam: 0, biaya: 0, laba: 0, kas: 0, jumlah: 0 };
  ...
  }, [owner, bookings, expenses, bulanIni]);
  ...
  }, [owner, bookings, bulanIni]);

  const belumLunas = useMemo(
    () => (owner ? (bookings || []).filter((b) => hitungBooking(b.items, b.dp, b.pelunasan).sisa > 0) : []),
    [owner, bookings]
  );
  const setoranPending = useMemo(
    () => (owner ? (bookings || []).filter((b) => b.statusSetoran === 'Pending' || b.statusSetoran === 'Belum') : []),
    [owner, bookings]
  );
  const setoranKosong = useMemo(
    () =>
      owner
        ? (bookings || []).filter((b) => (b.items || []).some((i) => i.jenisUnit === 'mitra' && !n(i.setoranOwner)))
        : [],
    [owner, bookings]
  );
  ...
  return (
    <div className="stack">
      {/* ====== HARI INI (semua peran lihat ini) ====== */}
      <div className="card">
        <div className="card-head">
          <div>
            <h2>Hari Ini</h2>
            <div className="small dim">
              {labelHariLengkap(hari)} · masuk sebagai {owner ? 'Owner' : 'Admin'}
            </div>
          </div>
      ...
      {owner && (
        <>
          <div className="grid grid-kpi">
            ...
            <Kpi label="Laba Kotor Travelink" angka={rupiahRingkas(ringkas.laba)} sub="nilai booking − setoran" />
            <Kpi label="Biaya" angka={rupiahRingkas(ringkas.biaya)} sub="bulan berjalan" />
            <Kpi label="Kas Bersih" angka={rupiahRingkas(ringkas.kas)} sub="laba kotor − biaya" />
          </div>
          ...
          <div className="card">
            <div className="card-head">
              <h2>Tren 12 Bulan</h2>
              <span className="small dim">Nilai booking & laba kotor</span>
            </div>
            <div style={{ width: '100%', height: 260 }}>
              <ResponsiveContainer>
                <BarChart data={tren} margin={{ top: 4, right: 4, left: -14, bottom: 0 }}>
                  ...
                  <Bar dataKey="laba" fill="var(--accent-light)" radius={[4, 4, 0, 0]} />
                </BarChart>
              </ResponsiveContainer>
            </div>
          </div>
